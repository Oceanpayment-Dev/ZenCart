...
class oceanpayment_cashu{
	...
	function oceanpayment_cashu() {	
		global $order;
		
		$this->code = 'oceanpayment_cashu';
		
		if ($_GET['main_page'] != '') {		
			$this->title = MODULE_PAYMENT_OCEANPAYMENT_CASHU_TEXT_CATALOG_TITLE; // Payment Module title in Catalog		
		} else {		
			$this->title = MODULE_PAYMENT_OCEANPAYMENT_CASHU_TEXT_ADMIN_TITLE; // Payment Module title in Admin		
		}
		
		$this->description = MODULE_PAYMENT_OCEANPAYMENT_CASHU_TEXT_DESCRIPTION;		
		$this->sort_order = MODULE_PAYMENT_OCEANPAYMENT_CASHU_SORT_ORDER;		
		$this->enabled = ((MODULE_PAYMENT_OCEANPAYMENT_CASHU_STATUS == 'True') ? true : false);
		
		if ((int)MODULE_PAYMENT_OCEANPAYMENT_CASHU_PENDING_STATUS_ID > 0) {		
			$this->order_status = MODULE_PAYMENT_OCEANPAYMENT_CASHU_PENDING_STATUS_ID;		
		}		
		
		if (is_object($order)) $this->update_status();
		
		$this->form_action_url = MODULE_PAYMENT_OCEANPAYMENT_CASHU_HANDLER;		
	...
	function update_status() {	
		...
		if ( ($this->enabled == true) && ((int)MODULE_PAYMENT_OCEANPAYMENT_CASHU_ZONE > 0) ) {
			$check_flag = false;	
			$check_query = $db->Execute("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_PAYMENT_OCEANPAYMENT_CASHU_ZONE . "' and zone_country_id = '" . $order->billing['country']['id'] . "' order by zone_id");
	...
	function selection() {	
		return array('id' => $this->code,
				'module' => MODULE_PAYMENT_OCEANPAYMENT_CASHU_TEXT_CATALOG_LOGO,
				'icon' => MODULE_PAYMENT_OCEANPAYMENT_CASHU_TEXT_CATALOG_LOGO
		);	
	}	
	...
	function confirmation() {	
		return array('title' => MODULE_PAYMENT_OCEANPAYMENT_CASHU_TEXT_DESCRIPTION);	
	}
	...
	function process_button() {	
		...
		//账户
		$account           = MODULE_PAYMENT_OCEANPAYMENT_CASHU_ACCOUNT;				
		//终端号
		$terminal          = MODULE_PAYMENT_OCEANPAYMENT_CASHU_TERMINAL;		
		//securecode
		$secureCode        = MODULE_PAYMENT_OCEANPAYMENT_CASHU_SCODE;				
		//返回地址
		$backUrl           = zen_href_link('checkout_oceanpayment_cashu', '', 'SSL');  // $backUrl = zen_href_link(FILENAME_CHECKOUT_PROCESS, '', 'SSL');
		//服务器响应地址
		$noticeUrl         = zen_href_link('checkout_oceanpayment_cashu', '', 'SSL');
		//支付方式
		$methods           = 'CashU';
		//订单号
	...
	function before_process() {	
		...
		if (strtolower($local_signValue) == strtolower($back_signValue)) {
			...
			if ($payment_status == 1) {				 
				//支付成功
				$this->order_status = MODULE_PAYMENT_OCEANPAYMENT_CASHU_SUCCESS_STATUS_ID;
				$comments           = 'Success.';
				...
				if($payment_authType == 1){
					...
					$this->order_status = MODULE_PAYMENT_OCEANPAYMENT_CASHU_WAITING_PROCESS_STATUS_ID;
				...
				//支付失败
				$this->order_status = MODULE_PAYMENT_OCEANPAYMENT_CASHU_FAILURE_STATUS_ID;
				$comments           = 'Failure.';
		...
		if(MODULE_PAYMENT_OCEANPAYMENT_CASHU_LOGS == 'True'){
	...
	function check() {	
		...
		if (!isset($this->_check)) {	
			$check_query = $db->Execute("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_PAYMENT_OCEANPAYMENT_CASHU_STATUS'");	
	...
	function install() {	
		...
		if (!defined('MODULE_PAYMENT_OCEANPAYMENT_CASHU_TEXT_CONFIG_1_1')) include(DIR_FS_CATALOG_LANGUAGES . $_SESSION['language'] . '/modules/' . $module_type . '/' . $this->code . '.php');
		
		//在数据库中插入模块设置(是否启用插件)
		$db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('" . MODULE_PAYMENT_OCEANPAYMENT_CASHU_TEXT_CONFIG_1_1 . "', 'MODULE_PAYMENT_OCEANPAYMENT_CASHU_STATUS', 'True', '" . MODULE_PAYMENT_OCEANPAYMENT_CASHU_TEXT_CONFIG_1_2 . "', '6', '0', 'zen_cfg_select_option(array(\'True\', \'False\'), ', now())");
		//在数据库中插入模块设置(填写账户)
		$db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('" . MODULE_PAYMENT_OCEANPAYMENT_CASHU_TEXT_CONFIG_3_1 . "', 'MODULE_PAYMENT_OCEANPAYMENT_CASHU_ACCOUNT', '', '" . MODULE_PAYMENT_OCEANPAYMENT_CASHU_TEXT_CONFIG_3_2 . "', '6', '2', now())");
		//在数据库中插入模块设置(填写终端号)
		$db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('" . MODULE_PAYMENT_OCEANPAYMENT_CASHU_TEXT_CONFIG_4_1 . "', 'MODULE_PAYMENT_OCEANPAYMENT_CASHU_TERMINAL', '', '" . MODULE_PAYMENT_OCEANPAYMENT_CASHU_TEXT_CONFIG_4_2 . "', '6', '4', now())");
		//在数据库中插入模块设置(终端安全码)
		$db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('" . MODULE_PAYMENT_OCEANPAYMENT_CASHU_TEXT_CONFIG_5_1 . "', 'MODULE_PAYMENT_OCEANPAYMENT_CASHU_SCODE', '', '" . MODULE_PAYMENT_OCEANPAYMENT_CASHU_TEXT_CONFIG_5_2 . "', '6', '6', now())");
		//在数据库中插入模块设置(支付地区)
		$db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, use_function, set_function, date_added) values ('" . MODULE_PAYMENT_OCEANPAYMENT_CASHU_TEXT_CONFIG_6_1 . "', 'MODULE_PAYMENT_OCEANPAYMENT_CASHU_ZONE', '0', '" . MODULE_PAYMENT_OCEANPAYMENT_CASHU_TEXT_CONFIG_6_2 . "', '6', '8', 'zen_get_zone_class_title', 'zen_cfg_pull_down_zone_classes(', now())");
		//在数据库中插入模块设置(默认订单状态)
		$db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, use_function, set_function, date_added) values ('" . MODULE_PAYMENT_OCEANPAYMENT_CASHU_TEXT_CONFIG_7_1 . "', 'MODULE_PAYMENT_OCEANPAYMENT_CASHU_PENDING_STATUS_ID', '1', '" . MODULE_PAYMENT_OCEANPAYMENT_CASHU_TEXT_CONFIG_7_2 . "', '6', '10', 'zen_get_order_status_name', 'zen_cfg_pull_down_order_statuses(', now())");
		//在数据库中插入模块设置(成功订单状态)
		$db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, use_function, set_function, date_added) values ('" . MODULE_PAYMENT_OCEANPAYMENT_CASHU_TEXT_CONFIG_8_1 . "', 'MODULE_PAYMENT_OCEANPAYMENT_CASHU_SUCCESS_STATUS_ID', '2', '" . MODULE_PAYMENT_OCEANPAYMENT_CASHU_TEXT_CONFIG_8_2 . "', '6', '12', 'zen_get_order_status_name', 'zen_cfg_pull_down_order_statuses(', now())");
		//在数据库中插入模块设置(失败订单状态)
		$db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, use_function, set_function, date_added) values ('" . MODULE_PAYMENT_OCEANPAYMENT_CASHU_TEXT_CONFIG_9_1 . "', 'MODULE_PAYMENT_OCEANPAYMENT_CASHU_FAILURE_STATUS_ID', '1', '" . MODULE_PAYMENT_OCEANPAYMENT_CASHU_TEXT_CONFIG_9_2 . "', '6', '14', 'zen_get_order_status_name', 'zen_cfg_pull_down_order_statuses(', now())");
		//在数据库中插入模块设置(待处理订单状态)
		$db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, use_function, set_function, date_added) values ('" . MODULE_PAYMENT_OCEANPAYMENT_CASHU_TEXT_CONFIG_10_1 . "', 'MODULE_PAYMENT_OCEANPAYMENT_CASHU_WAITING_PROCESS_STATUS_ID', '1', '" . MODULE_PAYMENT_OCEANPAYMENT_CASHU_TEXT_CONFIG_10_2 . "', '6', '16', 'zen_get_order_status_name', 'zen_cfg_pull_down_order_statuses(', now())");
		//在数据库中插入模块设置(支付模块排序)
		$db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('" . MODULE_PAYMENT_OCEANPAYMENT_CASHU_TEXT_CONFIG_11_1 . "', 'MODULE_PAYMENT_OCEANPAYMENT_CASHU_SORT_ORDER', '0', '" . MODULE_PAYMENT_OCEANPAYMENT_CASHU_TEXT_CONFIG_11_2 . "', '6', '18', now())");
		//在数据库中插入模块设置(支付提交地址)
		$db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('" . MODULE_PAYMENT_OCEANPAYMENT_CASHU_TEXT_CONFIG_12_1 . "', 'MODULE_PAYMENT_OCEANPAYMENT_CASHU_HANDLER', 'https://secure.oceanpayment.com/gateway/service/test', '" . MODULE_PAYMENT_OCEANPAYMENT_CASHU_TEXT_CONFIG_12_2 . "', '6', '20', '', now())");
		//在数据库中插入模块设置(响应代码)
		$db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('" . MODULE_PAYMENT_OCEANPAYMENT_CASHU_TEXT_CONFIG_14_1 . "', 'MODULE_PAYMENT_OCEANPAYMENT_CASHU_RESPONSE_CODE', 'false', '" . MODULE_PAYMENT_OCEANPAYMENT_CASHU_TEXT_CONFIG_14_2 . "', '6', '22', 'zen_cfg_select_option(array(\'Online\', \'Local\'), ', now())");
		//在数据库中插入模块设置（是否写日志）
        	$db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('" . MODULE_PAYMENT_OCEANPAYMENT_CASHU_TEXT_CONFIG_19_1 . "', 'MODULE_PAYMENT_OCEANPAYMENT_CASHU_LOGS', 'True', '" . MODULE_PAYMENT_OCEANPAYMENT_CASHU_TEXT_CONFIG_19_2 . "', '6', '2', 'zen_cfg_select_option(array(\'True\', \'False\'), ', now())");
	...
	function remove() {	
		global $db;	
		$db->Execute("delete from " . TABLE_CONFIGURATION . " where configuration_key LIKE  'MODULE_PAYMENT_OCEANPAYMENT_CASHU%'");	
	}
	
	
	function keys() {	
		return array(	
				'MODULE_PAYMENT_OCEANPAYMENT_CASHU_STATUS',
				'MODULE_PAYMENT_OCEANPAYMENT_CASHU_ACCOUNT',
				'MODULE_PAYMENT_OCEANPAYMENT_CASHU_TERMINAL',
				'MODULE_PAYMENT_OCEANPAYMENT_CASHU_SCODE',
				'MODULE_PAYMENT_OCEANPAYMENT_CASHU_ZONE',
				'MODULE_PAYMENT_OCEANPAYMENT_CASHU_PENDING_STATUS_ID',
				'MODULE_PAYMENT_OCEANPAYMENT_CASHU_SUCCESS_STATUS_ID',
				'MODULE_PAYMENT_OCEANPAYMENT_CASHU_FAILURE_STATUS_ID',
				'MODULE_PAYMENT_OCEANPAYMENT_CASHU_WAITING_PROCESS_STATUS_ID',
				'MODULE_PAYMENT_OCEANPAYMENT_CASHU_SORT_ORDER',
				'MODULE_PAYMENT_OCEANPAYMENT_CASHU_HANDLER',
				'MODULE_PAYMENT_OCEANPAYMENT_CASHU_RESPONSE_CODE',
                		'MODULE_PAYMENT_OCEANPAYMENT_CASHU_LOGS',
		);	
	}
